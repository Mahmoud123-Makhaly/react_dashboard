...
const BestSellingCustomersSalesChart = ({
  startDate,
  endDate,
}: {
  ...
  const t = useTranslate('COMP_BestSellingCustomersSalesChart');
  ...
  const loadData = useCallback(
    async filter => {
      ...
      apiClient.select<IDistributedColumnChartSeries>(`${endpoints.reports.topCustomerSalesStatistics}`, filter).then(
        data => {
          if (data && data.countStatus) {
            setData([
              {
                name: t('CHART_TOOLTIP_LABEL'),
                data: data.countStatus.map(order => ({ x: order.x || 'others', y: order.y || 0, meta: order.meta })),
              },
            ]);
          }
          setTimeout(() => {
...
export default BestSellingCustomersSalesChart;
