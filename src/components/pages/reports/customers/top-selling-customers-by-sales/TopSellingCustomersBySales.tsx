...
const TopSellingCustomersBySales = ({
  startDate,
  endDate,
}: {
  ...
  const t = useTranslate('COMP_SELLING_CUSTOMERS_BY_SALES');
  ...
  const loadData = useCallback(
    filter => {
      ...
      apiClient.select(endpoints.reports.topCustomerBySalesStatistics, filter).then(
        data => {
          if (data) {
            setData({
              headers: [
                ...
                { title: t('SALES'), field: 'sales' },
              ],
              data: data.map(x => ({
                sales: Utils.formatCash(x.sales) || 0,
...
export default TopSellingCustomersBySales;
