...
const BestSellingCustomersCountChart = ({
  startDate,
  endDate,
}: {
  ...
  const t = useTranslate('COMP_BestSellingCustomersCountChart');
  ...
  const loadData = useCallback(
    async filter => {
      ...
      apiClient.select<IDistributedColumnChartSeries>(`${endpoints.reports.topCustomerCountStatistics}`, filter).then(
        data => {
          if (data && data.countStatus) {
            setData([
              {
                name: t('CHART_TOOLTIP_LABEL'),
                data: data.countStatus.map(order => ({ x: order.x || '', y: order.y || 0, meta: order.meta })),
              },
            ]);
          }
          setTimeout(() => {
...
export default BestSellingCustomersCountChart;
