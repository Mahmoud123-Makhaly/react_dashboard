...
const OrderStatusSalesStatistics = ({
  ...
  const t = useTranslate('COMP_TaxesOrderStatusSalesStatistics');
  ...
  const loadData = useCallback(
    async filter => {
      ...
      apiClient
        .select<IPyramidChartSeries>(`${endpoints.reports.sales.taxes.orderStatusStatistics}${dataType}`, filter)
        .then(
          data => {
            if (data && data.name) {
              setData(
