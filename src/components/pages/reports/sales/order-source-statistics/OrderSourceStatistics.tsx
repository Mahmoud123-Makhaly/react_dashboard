...
const OrderSourceStatistics = ({
  ...
  const t = useTranslate('COMP_OrderSourceStatistics');
  ...
  const loadData = useCallback(
    async filter => {
      ...
      apiClient.select<IPyramidChartSeries>(`${endpoints.reports.orderSourceStatistics}${dataType}`, filter).then(
        data => {
          if (data && data.name) {
            setData(
              data.results
...
export default OrderSourceStatistics;
