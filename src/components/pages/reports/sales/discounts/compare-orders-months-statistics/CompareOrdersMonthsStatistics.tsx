...
const CompareOrdersMonthsStatistics = ({ dataType = 'Sales' }: { dataType?: 'Sales' | 'Count' }) => {
  ...
  const t = useTranslate('COMP_ReportSalesDiscountCompareOrdersMonthsStatistics');
  ...
  const loadData = useCallback(async () => {
    ...
    apiClient
      .select<IDistributedColumnChartSeries>(`${endpoints.reports.sales.discounts.monthlyStatistics}${dataType}`)
      .then(
        data => {
          if (data && data.allOrder && data.completedOrder && data.inCompletedOrder) {
            ...
            const unionData = Utils.unifyBy<{ x: string; y: number; meta: any }>(series.map(obj => ({ name: obj.type, data: obj.data })), 'x', { 'x': '', 'y': 0, 'meta': null },(data)=>data.sort((a,b)=>Utils.getMonthNumberFromName(a.x)>Utils.getMonthNumberFromName(b.x)?1:-1));
