...
const IncompleteOrdersStatistics = ({
  ...
  const t = useTranslate('COMP_ReportSalesDiscountIncompleteOrdersStatistics');
  ...
  const loadData = useCallback(
    async (
      ...
      new Promise((resolve, reject) =>
        apiClient
          .select<IDonutChartSeries>(`${endpoints.reports.sales.discounts.orderStatusStatistics}${dataType}`, filter)
          .then(
            data => {
              if (data && data.name) {
                resolve({
