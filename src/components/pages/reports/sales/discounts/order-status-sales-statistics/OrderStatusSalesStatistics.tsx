...
const OrderStatusSalesStatistics = ({
  ...
  const t = useTranslate('COMP_ReportSalesDiscountsOrderStatusSalesStatistics');
  ...
  const loadData = useCallback(
    async filter => {
      ...
      apiClient
        .select<IPyramidChartSeries>(`${endpoints.reports.sales.discounts.orderStatusStatistics}${dataType}`, filter)
        .then(
          data => {
            if (data && data.name) {
              setData(
