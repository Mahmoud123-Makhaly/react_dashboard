...
const OrderBySourceSalesStatistics = ({
  dataType = 'Sales',
  startDate,
  endDate,
}: {
  ...
  const t = useTranslate('COMP_ReportSalesDiscountsOrderSource');
  ...
  const loadData = useCallback(
    async filter => {
      ...
      apiClient
        .select<IDistributedColumnChartSeries>(`${endpoints.reports.sales.discounts.orderBySource}${dataType}`, filter)
        .then(
          data => {
            if (data && data.name) {
              setData([
...
export default OrderBySourceSalesStatistics;
