...
const AnimatedNumbersStatistics = ({
  ...
  const t = useTranslate('COMP_ReportSalesDiscountsAnimatedNumbersStatistics');

  const loadOrderDashboardStatistics = useCallback(
    async filter => {
      apiClient.select<IOrderStatistics>(endpoints.reports.sales.discounts.orderStatistics, filter).then(
