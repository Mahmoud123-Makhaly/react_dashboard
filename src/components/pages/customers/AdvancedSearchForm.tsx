...
import { customerOuterIdLookup } from '@app/libs';
import { FormikHelpers, FormikValues } from 'formik';
interface ICustomerAdvancedSearchFormProps {
  initialValues?: any;
  onCancel: () => void;
  onSubmit: (values: FormikValues, formikHelpers: FormikHelpers<FormikValues>) => void | Promise<any>;
}

const CustomerAdvancedSearchForm = (props: ICustomerAdvancedSearchFormProps) => {
  ...
  const t = useTranslate('COMP_CustomerAdvancedSearchForm');
  const toast = useToast();
  const lookup = useLookup([customerOuterIdLookup]);
  //Form fields
  ...
  useEffect(() => {
    lookup.load().then(
      data => {
        defaultFormFields.find(x => x.name === 'outerIds')!.options = [...data.customersOuterId];
