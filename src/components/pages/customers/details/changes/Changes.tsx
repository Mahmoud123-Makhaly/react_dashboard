...
import { ILog, ILogsList } from '@app/types';
import { endpoints } from '@app/libs';
import { Utils } from '@helpers/utils';

const CustomerChanges = ({ customerId }: { customerId?: string }) => {
  const [loadingStatus, setLoadingStatus] = useState<DataLoadingStatus>();
  const [sortedColumn, setSortedColumn] = useState<SortedColumn>();
  const apiClient = useAPIAuthClient();
  const toast = useToast();
  const t = useTranslate('COMP_CustomerDetails.CHANGES');
  ...
  }, [customerId, defaultSortedColumn]);
  ...
  const exportFile: IExportFile = {
    fileName: 'customer-changes',
    endpoint: endpoints.logs.list,
    method: 'search',
  ...
  const loadData = useCallback(async () => {
    apiClient.search<ILogsList>(endpoints.logs.list, { ...filter }).then(
      data => {
        if (data) {
          setDataSource(prev => ({
            ...prev,
