...
import { catalogLookup, storesLookup, priceListLookup } from '@app/libs';
import { FormikHelpers, FormikValues } from 'formik';
interface IPricesAssignmentsAdvancedSearchFormProps {
  initialValues?: any;
  onCancel: () => void;
  onSubmit: (values: FormikValues, formikHelpers: FormikHelpers<FormikValues>) => void | Promise<any>;
}

const PricesAssignmentsAdvancedSearchForm = (props: IPricesAssignmentsAdvancedSearchFormProps) => {
  ...
  const t = useTranslate('COMP_PricesAssignmentsAdvancedSearchForm');
  const toast = useToast();
  const lookup = useLookup([catalogLookup, storesLookup, priceListLookup]);
  //Form fields
  ...
  useEffect(() => {
    lookup.load().then(
      data => {
        ...
        defaultFormFields.find(x => x.name === 'priceListIds')!.options = [...data.priceList];
...
export default PricesAssignmentsAdvancedSearchForm;
