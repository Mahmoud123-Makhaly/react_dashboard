...
const ProductSalesStatistics = ({
  ...
  const t = useTranslate('COMP_ProductSalesStatistics');
  ...
  const loadData = useCallback(
    async filter => {
      ...
      apiClient
        .select<IDistributedColumnChartSeries>(`${endpoints.dashboard.orderProductStatistics}${dataType}`, filter)
        .then(
          data => {
            if (data && data.name) {
              setData([
...
export default ProductSalesStatistics;
