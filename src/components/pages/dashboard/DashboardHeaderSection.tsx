...
const DashboardHeaderSection = ({ onSubmit }: { onSubmit: (values, setSubmitting) => void }) => {
  const { data: session, status } = useSession();
  const [dataType, setDataType] = useState<'Sales' | 'Count'>('Sales');
  const t = useTranslate('COMP_DashboardHeaderSection');
      ...
      <Col xs={12}>
        <div className="d-flex align-items-lg-center flex-lg-row flex-column">
          <div className="flex-grow-1">
            <h4 className="fs-16 mb-1">{t('WELCOME_MSG', { userName: session?.user.userName })}</h4>
          </div>
