...
const AnimatedNumbersStatistics = ({
  ...
  const t = useTranslate('COMP_AnimatedNumbersStatistics');

  const loadOrderDashboardStatistics = useCallback(
    async filter => {
      apiClient.select<IOrderStatistics>(endpoints.dashboard.orderStatistics, filter).then(
  ...
  const loadERPOrderDashboardStatistics = useCallback(
    async filter => {
      apiClient.select<IOrderStatistics>(endpoints.dashboard.erpOrderStatistics, filter).then(
        (data: IOrderStatistics | null) => {
          if (data) {
            const result: Array<IMiniAttachedAnimatedCardNumbersProps> = [
              {
                title: t('ALL_ERP_ORDERS_SALES'),
                count: data.allOrderSales || 0,
                icon: <FeatherIcon icon="dollar-sign" className="display-6 text-muted" />,
              },
              {
                title: t('COMPLETED_ERP_ORDERS_SALES'),
                count: data.completedOrderSales || 0,
                icon: <FeatherIcon icon="check-circle" className="display-6 text-muted" />,
              },
              {
                title: t('ALL_ERP_ORDERS_COUNT'),
                count: data.allOrderCount || 0,
                isDecimal: false,
                icon: <FeatherIcon icon="shopping-cart" className="display-6 text-muted" />,
              },
              {
                title: t('COMPLETED_ERP_ORDERS_COUNT'),
                count: data.completedOrderCount || 0,
                isDecimal: false,
                icon: <FeatherIcon icon="shopping-bag" className="display-6 text-muted" />,
              },
              {
                title: t('AVG_ALL_ERP_ORDERS_SALES'),
                count: data.avgAllOrderSales || 0,
                icon: <FeatherIcon icon="shopping-cart" className="display-6 text-muted" />,
              },
              {
                title: t('AVG_COMPLETED_ERP_ORDERS_SALES'),
                count: data.avgCompletedOrderSales || 0,
                icon: <FeatherIcon icon="shopping-bag" className="display-6 text-muted" />,
              },
            ];
            setERPOrderDashboardStatisticsData(result);
          }
          setTimeout(() => {
  ...
  useEffect(() => {
    ...
    loadERPOrderDashboardStatistics({ start, end });
  }, [startDate, endDate, loadOrderDashboardStatistics, loadERPOrderDashboardStatistics]);
      ...
      <Row>
        <div className="col-xl-12">
          <div className="card crm-widget">
            <div className="card-body p-0">
              <div className="row row-cols-xxl-6 row-cols-md-3 row-cols-1 g-0">
                {erpOrderDashboardStatisticsData.map((item, indx) => (
                  <Col key={'order-statistics-chunk' + indx}>
                    <MiniAttachedAnimatedCardNumbers {...item} isLoading={isERPLoading} />
                  </Col>
                ))}
              </div>
            </div>
