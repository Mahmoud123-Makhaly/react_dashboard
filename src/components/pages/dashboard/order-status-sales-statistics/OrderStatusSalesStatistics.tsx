...
const OrderStatusSalesStatistics = ({
  ...
  const t = useTranslate('COMP_OrderStatusSalesStatistics');
  ...
  const loadData = useCallback(
    async filter => {
      ...
      apiClient.select<IPyramidChartSeries>(`${endpoints.dashboard.orderStatusStatistics}${dataType}`, filter).then(
        data => {
          if (data && data.name) {
            setData(
              data.results
