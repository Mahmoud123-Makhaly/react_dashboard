...
const ProductCategorySalesStatistics = ({
  dataType = 'Sales',
  startDate,
  endDate,
}: {
  ...
  const t = useTranslate('COMP_ProductCategorySalesStatistics');
    ...
    async filter => {
      ...
      apiClient
        .select<IDistributedColumnChartSeries>(
          `${endpoints.dashboard.orderProductCategoriesStatistics}${dataType}`,
          filter,
        )
        .then(
          data => {
...
export default ProductCategorySalesStatistics;
