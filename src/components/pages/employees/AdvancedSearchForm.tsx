...
import { employeesOuterIdLookup } from '@app/libs';
import { FormikHelpers, FormikValues } from 'formik';
interface IEmployeesAdvancedSearchFormProps {
  initialValues?: any;
  onCancel: () => void;
  onSubmit: (values: FormikValues, formikHelpers: FormikHelpers<FormikValues>) => void | Promise<any>;
}

const EmployeesAdvancedSearchForm = (props: IEmployeesAdvancedSearchFormProps) => {
  ...
  const t = useTranslate('COMP_EmployeesAdvancedSearchForm');
  const toast = useToast();
  const lookup = useLookup([employeesOuterIdLookup]);
  //Form fields
  ...
  useEffect(() => {
    lookup.load().then(
      data => {
        defaultFormFields.find(x => x.name === 'outerIds')!.options = [...data.employeesOuterId];
...
export default EmployeesAdvancedSearchForm;
