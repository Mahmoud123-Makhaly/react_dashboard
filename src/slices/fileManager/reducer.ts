import { CreateSliceOptions, SliceCaseReducers, createSlice } from '@reduxjs/toolkit';
import {
  getFolders,
  addNewFolder,
  updateFolder,
...
const FileManagerSlice = createSlice({
  ...
  extraReducers: builder => {
    ...
    builder.addCase(deleteFolder.fulfilled, (state: any, action: any) => {
      state.folders = state.folders.filter(folder => folder.id.toString() !== action.payload.id.toString());
    });
    ...
    builder.addCase(updateFile.fulfilled, (state: any, action: any) => {
      state.files = state.files.map(files =>
        files.id.toString() === action.payload.id.toString() ? { ...files, ...action.payload } : files,
      );
    });
    ...
    builder.addCase(deleteFile.fulfilled, (state: any, action: any) => {
      state.files = state.files.filter(file => file.id.toString() !== action.payload.id.toString());
    });
