  ...
  extraReducers: builder => {
    ...
    builder.addCase(updateProduct.fulfilled, (state: any, action: any) => {
      state.products = state.products.map(product =>
        product._id.toString() === action.payload._id.toString() ? { ...product, ...action.payload } : product,
      );
    });
    ...
    builder.addCase(updateOrder.fulfilled, (state: any, action: any) => {
      state.orders = state.orders.map(order =>
        order._id.toString() === action.payload._id.toString() ? { ...order, ...action.payload } : order,
      );
    });
    ...
    builder.addCase(deleteOrder.fulfilled, (state: any, action: any) => {
      state.orders = state.orders.filter(order => order._id.toString() !== action.payload.order.toString());
    });
    ...
    builder.addCase(updateCustomer.fulfilled, (state: any, action: any) => {
      state.customers = state.customers.map(customer =>
        customer._id.toString() === action.payload._id.toString() ? { ...customer, ...action.payload } : customer,
      );
    });
